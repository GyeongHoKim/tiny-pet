...
func TestEchoMicrosecondsToDistanceCm(t *testing.T) {
	tests := []struct {
		name string
		us   int
		want int
		...
		{"58 µs = 1 cm", 58, 1},
		{"116 µs = 2 cm", 116, 2},
		{"1160 µs = 20 cm", 1160, 20},
		{"0 µs = timeout", 0, TimeoutDistance},
		{"-1 µs = timeout", -1, TimeoutDistance},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := EchoMicrosecondsToDistanceCm(tt.us)
			if got != tt.want {
				t.Errorf("EchoMicrosecondsToDistanceCm(%d) = %d, want %d", tt.us, got, tt.want)
			}
		})
	}
}
